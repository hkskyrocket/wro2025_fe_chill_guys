...
def getColourDetected(red,green,blue):
    red = int(red)
    green = int(green)
    blue = int(blue)
    if(red>500):
        currentDetectedColout = colour[2]
    elif(blue>500):
        currentDetectedColout = colour[3]
    else:
        currentDetectedColout = colour[1]

def rightTurn():
    sound.beep()
    while(getWallDistance(horDis.distance_centimeters)<40):
        wallFollower
    sound.beep()
    while (getHeading(gyro.angle)<90):
        steeringMotor.on_to_position(SpeedPercent(100),int(12 + steeringInit ))
